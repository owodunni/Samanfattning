...
\item För att kunna teckna den totala kraften F på en masspunkt, måste man frilägga, dvs isolera masspunkten från sin omgivning.

\end{itemize}

\subsection{System av flera masspunkter, Newtons tredje lag}
Om man har flera masspunkter så är det oftast lämpligt att studera ett system av alla masspunkter. Den totala kraften kan då täcknas som:

\begin{equation}
F_{ext} =\sum F_i=\sum \dot{p}=\sum ma
\end{equation}

\subsection{Exempel på krafter}
Olika typer av krafter

\subsubsection{Tyngdkraft}
\begin{equation}
F_g = mg
\end{equation}

\subsubsection{Snörkraft}
Snörkraften är överallt lika stor och betäcknas som $S$.

\subsubsection{Fjärderkraft}
...
\subsubsection{Stångkraft}
Sången som stångkraften verkar genom är oändligt styv men funkar annars som snörkraften.

\subsubsection{Dämparkraft}

\begin{equation}
F_d=c\dot{x}
\end{equation}

\subsubsection{Strömningsmotstånd}
Bryr mig inte..

\subsubsection{Friktion}

$\micro_k < \micro_s$

Vid grepp:
\begin{equation}
\abs{F}\leq \micro_s\abs{N}
\end{equation}

Vid glid:
\begin{equation}
\abs{F}= -\micro_k\abs{N}\frac{v}{\abs{v}}
\end{equation}

Samma ekvation som åvan dock är det här underförstått att friktionskraften vid glidning är motriktad glidningsriktningen.
\begin{equation}
\abs{F}= \micro_k\abs{N}
\end{equation}
